...
public class JwtTokenProvider {
    ...
    public Authentication getAuthentication(String token) {
        LOGGER.info("[JwtTokenProvider] 토큰 인증 정보 조회 시작");
        UserDetails userDetails = userDetailsService.loadUserByUsername(this.getUsername(token));
        LOGGER.info("[JwtTokenProvider] 토큰 인증 정보 조회 완료, UserDetails UserName : {}",
    ...
    public String getUsername(String token) {
        LOGGER.info("[JwtTokenProvider] 토큰 기반 회원 구별 정보 추출");
        String info = Jwts.parser().setSigningKey(secretKey).parseClaimsJws(token).getBody()
                .getSubject();
        LOGGER.info("[JwtTokenProvider] 토큰 기반 회원 구별 정보 추출 완료, info : {}", info);
        return info;
    }
    ...
    public boolean validateToken(String token) {
        LOGGER.info("[JwtTokenProvider] 토큰 유효 체크 시작");
        try {
